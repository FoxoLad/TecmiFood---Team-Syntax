//Avisos del usuario: Cada cambio de estado del pedido aparece aquí.
...
const getStatusMessage = (status: string, orderNumber: string) => {
  ...
  if (status === "En preparación")
    return `Tu pedido #${orderNumber} ya se está preparando.`;
  if (status === "Terminado")
...
export default function NotificationsScreen() {
  ...
  const deletedNotificationIds = useUserStore(
    (state) => state.deletedNotificationIds,
  );
      ...
      >
        <View style={{ flex: 1, paddingRight: 10 }}>
          ...
          <Text style={styles.subtitle}>
            Te avisamos cada vez que tu pedido cambie de estado.
          </Text>
        </View>
        {alerts.length > 0 && (
          <Pressable
      ...
      {alerts.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyMessage}>
            Cuando tu pedido se reciba, se prepare o esté listo, te llegará
            aquí.
          </Text>
        </View>
      ) : (
